...
function tab_bar_add_buffer (buffer) {
    ...
    // user wants.
    var iconlabel = create_XUL(buffer.window, "image");
    iconlabel.setAttribute("class", "tab2-icon");
    if (buffer.favicon != null)
        image.setAttribute("src", buffer.favicon);
...
function tab_bar_update_buffer_icon (b) {
    if (b.favicon != null)
        b.tab.tab_icon.setAttribute("src", b.favicon);
    else
        b.tab.tab_icon.removeAttribute("src");
}
